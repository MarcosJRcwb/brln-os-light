...
func (s *Service) RunDaily(ctx context.Context, reportDate time.Time, loc *time.Location, rebalanceOverride *RebalanceOverride, paymentOverride *PaymentOverride, keysendOverride *KeysendReceivedOverride) (Row, error) {
	tr := BuildTimeRangeForDate(reportDate, loc)
	metrics, err := ComputeMetrics(ctx, s.lnd, tr, false, rebalanceOverride, paymentOverride, keysendOverride)
	if err != nil {
		return Row{}, err
	}
...
func (s *Service) Live(ctx context.Context, now time.Time, loc *time.Location, lookbackHours int) (TimeRange, Metrics, error) {
	...
	metrics, err := ComputeMetrics(ctx, s.lnd, tr, false, nil, nil, nil)
	if err != nil {
